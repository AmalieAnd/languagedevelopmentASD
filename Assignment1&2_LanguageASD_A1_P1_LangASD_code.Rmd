...
# Homogenising the way visits are reported - now it's only numbers. str.extract is used to only look for numbers, which then renames the values to only numbers
...
# MLU less than 1,5 for the first visits
clean_train %>%
...
```{r}
# Ordered by the children who produced the most words on the 6th visit
clean_train %>%
  filter(Visit == "6") %>%
  arrange(tokens_CHI)

# Ordered by the children who produced the least amount of words on the 1st visit
clean_train %>%
  filter(Visit == "1") %>%
