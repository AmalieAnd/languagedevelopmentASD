...
library(pacman)
p_load(tidyverse, data.table, stringr, ggplot2, Metrics, lmerTest, caret, gdata, plyr)
...
# Homogenising the way visits are reported - now it's only numbers. str.extract is used to only look for numbers, which then renames the values to only numbers. It's extracting all the values that match the pattern I have specified in the square brackets: numbers between 1-6
...
```{r}
# Replacing all punctuations with nothing - leaving out only the letters
...
## Other ways
# str.detect + str.remove ".$"
# the dollar sign tells R to look for punctuations at the end of sentences --> $ is a way to specify ends
```
...
```{r}
# First selecting each variable needed from each dataset
subdemo <- select(demo_train, c(ID, Visit, Ethnicity, Diagnosis, Gender, Age, ADOS,  MullenRaw, ExpressiveLangRaw))
sublu <- select(LU_train, c(ID, Visit, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd))
subtoken <- select(token_train, c(ID, Visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI))

## Note: we didn't rename the last two variables
```
...
```{r}
# Merging datasets
train <- join(sublu, subtoken) 
wholetrain <- join(subdemo, train) 

#Joining by ID and Visit
...
```{r}
# Subsetting data consistning of only the variables values from visit 1
subset_train <- subset(wholetrain, Visit == 1, select = (c(ID,ADOS, MullenRaw, ExpressiveLangRaw)))

# Using select and then filter to make a dataset only consisting of the data from visit 1
select_train <- select(wholetrain, c(Visit, ADOS, MullenRaw, ExpressiveLangRaw))
select_train <- filter(select_train, Visit == 1)

# Changing variable names in order to not overwrite in grand dataset
subset_train <- rename.vars(subset_train,"ADOS", "ADOS1")
subset_train <- rename.vars(subset_train,"MullenRaw", "MullenRaw1")
subset_train <- rename.vars(subset_train,"ExpressiveLangRaw", "ExpressiveLangRaw1")

# Merging 
train <- join(wholetrain, subset_train) 

# Filtering out old variables as well as ordering the variables in the right order
...
```{r}
# 1. Filtering out participants with a Mean Length of Utterance value larger than 2.7 morphemes
clean_train %>%
  filter(CHI_MLU > 2.7)

# 2. Filtering out participants with a MLU less than 1,5 for the first visits
clean_train %>%
  filter(CHI_MLU < 1.5, Visit == "1")

# 3. Filtering all kids that have a visit with NA values (since in MLU, NA is in all)
clean_train %>%
...
```{r}
# 1. Ordered by the children who produced the most words on the 6th visit
clean_train %>%
  filter(Visit == 6) %>%
  arrange(desc(tokens_CHI)) %>%
  slice(1)

# 2. Ordered by the children who produced the least amount of words on the 1st visit
clean_train %>%
  filter(Visit == "1") %>%
  arrange(desc(tokens_CHI))
...
onlyASD <- subset(clean_train, Diagnosis == "ASD", select = (c("Diagnosis", "CHI_MLU", "tokens_CHI")))

# 2. Repeating
onlyASD <- subset(clean_train, Diagnosis == "ASD", select = (c("Diagnosis", "Diagnosis", "CHI_MLU", "tokens_CHI")))

# Nothing happens - only one of the variables is íncluded
...
3. The solution to the task above enables us to assess the average amount of words produced by each child. Why don't we just use these average values to describe the language production of the children? What is the advantage of keeping all the data?

```{r}
#Problems with the plyr package - couldn't use group_by
detach(package:plyr)
library(dplyr)

# Changing 
clean_train$ID <- as.character(clean_train$ID)


# 1: Adding a coloumn with an overall mean value of all words spoken (all participants, all trials)
clean_train <- clean_train %>%
  mutate(mean_overall = mean(tokens_CHI, na.rm = T))

# 2: Adding a coloumn with the value of each participant's mean number of words spoken generally 
clean_train <- clean_train %>%
  group_by(ID) %>%
  mutate(mean_tokens = mean(tokens_CHI, na.rm = T))

# 3. You lose a lot of information by filtering out any other variables than the average, so even though the average is good for gaining an overview of the data, the interpretation will be simplistic. If there's no normal distribution, then the mean is not necessarily a good description of the data we have before us - it will hide each participant's actual progress. Keeping all the data is more complicated though, and it requires more steps and energy to understand the data. What you get out of keeping it though, is all the variance 
